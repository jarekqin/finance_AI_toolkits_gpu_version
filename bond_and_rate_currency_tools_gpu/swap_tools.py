...
def swap_rate(frequency, rate_on_delivery, maturity):
    ...
    :return: rate
    """
    n_list = cp.arange(1, frequency * maturity + 1)
    t = n_list / frequency
    q = cp.exp(-rate_on_delivery * maturity)
    rate = frequency * (1 - q[-1]) / cp.sum(q) if len(q) > 1 else frequency * (1 - q) / cp.sum(q)
    return rate
...
def swap_value(fix_leg, float_leg, maturity, ytm_rate, frequency, principle, position):
    ...
    :return: swap value
    """
    b_fix = (fix_leg * cp.sum(cp.exp(-ytm_rate * maturity)) / frequency + cp.exp(
        ytm_rate[-1] * maturity[-1])) * principle if len(ytm_rate) > 1 and len(maturity) > 1 else \
        (fix_leg * cp.sum(cp.exp(-ytm_rate * maturity)) / frequency + cp.exp(
            ytm_rate * maturity)) * principle
    b_float = (float_leg / frequency + 1) * principle * cp.exp(ytm_rate[0] * maturity[0]) if len(ytm_rate) > 1 and len(
        maturity) > 1 \
        else (float_leg / frequency + 1) * principle * cp.exp(ytm_rate * maturity)
    if position == 'long':
...
def CCS_double_fixed_cashflow(cur1, cur2, leg_rate1, leg_rate2, frequency, maturity, trader, principle):
    ...
    """
    cashflow = cp.zeros(frequency * maturity + 1)
...
def CCS_value(types, leg1, leg2, rate1, rate2, ytm1, ytm2, spot_rate, frequency, maturity, trader):
    ...
    if types == "double_fixed":
        bond_a = (rate1 * cp.sum(cp.exp(-ytm1 * maturity)) / frequency + cp.exp(-ytm1[-1] * maturity[-1])) * leg1
        bond_b = (rate2 * cp.sum(cp.exp(-ytm2 * maturity)) / frequency + cp.exp(-ytm2[-1] * maturity[-1])) * leg2
        if trader not in ['A', 'a']:
            swap_value = bond_a - bond_b * spot_rate
        else:
            swap_value = bond_b - bond_a / spot_rate
    elif types == 'double_float':
        bond_a = (rate1 / frequency + 1) * cp.exp(-ytm1[0] * maturity[0]) * leg1
        bond_b = (rate2 / frequency + 1) * cp.exp(-ytm2[0] * maturity[0]) * leg2
        if trader not in ['A', 'a']:
            swap_value = bond_a - bond_b * spot_rate
        else:
            swap_value = bond_b - bond_a / spot_rate
...
if __name__ == '__main__':
    ...
    par = 1e8
    frequency = 2
    rate_float = cp.array([0.03197, 0.032, 0.029823, 0.030771, 0.04451, 0.047093, 0.04304, 0.03275, 0.02963, 0.01566])
    print(irs(rate_float, rate_fix, par, frequency, 'long'))
    print(irs(rate_float, rate_fix, par, frequency, 'short'))
    r_list = cp.array([0.020579, 0.021276, 0.022080, 0.022853, 0.023527, 0.024036])
    print(swap_rate(2, r_list, 3))
    par = 1e8
    rate_fix = 0.0241
    rate_float = 0.02178
    frequency = 3
    t_list = cp.array([0.47945205, 0.97945205, 1.47945205, 1.97945205, 2.47945205, 2.97945205])
    ytm_list = cp.array([0.027564, 0.022548, 0.03125, 0.014755, 0.021432, 0.0268853])
    ...
    maturity = 3
    rate_fix = 0.03
    libor = cp.array([0.01291, 0.014224, 0.016743, 0.024744, 0.028946, 0.0251661])
    ...
    frequency = 1
    maturity = 4
    shibor = cp.array([0.0316, 0.046329, 0.03527, 0.03122])
    hibor = cp.array([0.013295, 0.015057, 0.026593, 0.023743])
    ...
    rmb = usd * 7.0771
    libor = 0.010024
    ytm_rmb = cp.array([0.021156, 0.023294, 0.023811])
    ytm_usd = cp.array([0.0019, 0.0019, 0.0022])
    t_list = cp.array([0.78630137, 1.78630137, 2.78630137])
