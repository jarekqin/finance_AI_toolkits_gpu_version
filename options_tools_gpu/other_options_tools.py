...
def pd_merton(stock_value, debt_value, company_value, sigma, risk_free, maturity):
    ...
    :return: probability
    """
    d1 = (cp.log(company_value / debt_value) + (risk_free + pow(sigma, 2) / 2 * maturity)) / (
            sigma * cp.sqrt(maturity)
    )
    d2 = d1 - sigma * cp.sqrt(maturity)
    return norm.cdf(-d2.get())
...
def convert_bond_binal_tree(stock_price, sigma, principle, convert_ratio, PD, risk_free, recovery_rate,
    ...
    """
    t = maturity / steps
    u = cp.exp(cp.sqrt((pow(sigma, 2) - PD) * t))
    d = 1 / u
    pu = (cp.exp(risk_free * t) - d * cp.exp(-PD * t)) / (u - d)
    pd = (u * cp.exp(-PD * t) - cp.exp(risk_free * t)) / (u - d)
    p_default = 1 - cp.exp(-PD * t)
    d_value = principle * recovery_rate
    cb_matrix = cp.zeros((steps + 1, steps + 1))
    n_list = cp.arange(0, steps + 1)
    s_end = stock_price * pow(u, steps - n_list) * pow(d, n_list)
    Q1 = principle
    Q3 = convert_ratio * s_end
    cb_matrix[:, -1] = cp.maximum(cp.minimum(Q1, return_price), Q3)
    i_list = list(range(0, steps))
    i_list.reverse()
    for i in i_list:
        j_list = cp.arange(i + 1)
        si = stock_price * pow(u, i - j_list) * pow(d, j_list)
        Q1 = cp.exp(-risk_free * t) * (
                pu * cb_matrix[:i + 1, i + 1] + pd * cb_matrix[1:i + 2, i + 1] + p_default * d_value)
        Q3 = convert_ratio * si
        cb_matrix[:i + 1, i] = cp.maximum(cp.minimum(Q1, return_price), Q3)
    return cb_matrix[0, 0].get()
...
def bsm_futures_options(futures_price, strike_price, sigma, risk_free, maturity, options_type):
    ...
    :return: options price
    """
    d1 = (cp.log(futures_price / strike_price) + pow(sigma, 2) * maturity / 2) / (sigma * cp.sqrt(maturity))
    d2 = d1 - sigma * cp.sqrt(maturity)
    if options_type in ['call', 'CALL', 'Call']:
        return cp.exp(-risk_free * maturity) * (futures_price * norm.cdf(d1.get()) - strike_price * norm.cdf(d2.get()))
    elif options_type in ['put', 'PUT', 'Put']:
        return (cp.exp(-risk_free * maturity) * (
                strike_price * norm.cdf(-d2.get()) - futures_price * norm.cdf(-d1.get()))).get()
...
def caplet(principle, risk_free, forward_rate, strike_price, sigma, time1, time2):
    ...
    """
    d1 = (cp.log(forward_rate / strike_price) + 0.2 * pow(sigma, 2) * time1) / (sigma * cp.sqrt(time1))
    d2 = d1 - sigma * cp.sqrt(time1)
    tau = time2 - time1
    return (principle * tau * cp.exp(-risk_free * time2) * (
            forward_rate * norm.cdf(d1.get()) - strike_price * norm.cdf(d2.get()))).get()
...
def floorlet(principle, risk_free, forward_rate, strike_price, sigma, time1, time2):
    ...
    :return: price of caplet
    """
    d1 = (cp.log(forward_rate / strike_price) + pow(sigma, 2) * time1 / 2) / (sigma * cp.sqrt(time1))
    d2 = d1 - sigma * cp.sqrt(time1)
    tau = time2 - time1
    return (principle * tau * cp.exp(-risk_free * time2) * (
                strike_price * norm.cdf(-d2.get()) - forward_rate * norm.cdf(-d1.get()))).get()
...
def swapoption(principle, forward_rate, fix_rate, frequency, sigma, options_maturity, swap_maturity, risk_free_list,
    ...
    """
    d1 = (cp.log(forward_rate / fix_rate) + pow(sigma, 2) * options_maturity / 2) / (sigma * cp.sqrt(options_maturity))
    d2 = d1 = sigma * cp.sqrt(options_maturity)
    t_list = options_maturity + cp.arange(1, frequency * swap_maturity + 1) / frequency
    if direction == 'pay':
        value = cp.sum(cp.exp(-risk_free_list * t_list) * principle * (
...
def foward_swaprates(swap_rate_list, options_maturity, swap_maturity, frequency):
    ...
    """
    t_list = frequency * cp.arange(1, frequency * swap_maturity + 1)
    ...
    b = (1 / options_maturity) * cp.sum(pow(1 + swap_rate_list[1:] / frequency, -t_list))
    return a.get() / b.item()
